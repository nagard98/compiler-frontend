...
parseTree (Prog pBlock dclBlock beBlock _) env errs = do
    ...
    (finalEnv, errors2, beBlks) <- parseBEBlock globEnv errors1 beBlock GlobalBlk
...
parseDclFcBlock env errors (DclBlockFcBlock fB@(FuncBlock idTok@(TokIdent (pos, id)) params retType beb)) = do
    ...
    (finalEnv, finalErrors, annotatedBEB) <- parseBEBlock tmpEnv2 tmpErrors2 beb FunctionBlk
...
parseDclPcBlock env errors (DclBlockPcBlock pB@(ProcBlock idTok@(TokIdent (pos, id)) params beb)) = do
    ...
    (fEnv, fErrs, annBEB) <- parseBEBlock pEnv pErrs beb ProcedureBlk
...
parseBEBlock:: Env -> Errors -> BEBlock env infType -> BlockType -> SSAState (Env, Errors, BEBlock Env Type)
parseBEBlock env errors (BegEndBlock statements annEnv) blkType = do
    pushNewUninit
    (newEnv, newErrors, newStatements) <- parseStatements env errors statements blkType
    newErrors2 <- popUninit newErrors
    return (newEnv, newErrors2, BegEndBlock newStatements newEnv)


parseStatements:: Env -> Errors -> [Stmt env infType] -> BlockType -> SSAState (Env, Errors, [Stmt Env Type])
parseStatements env errors [] blkType = return (env, errors, [])
parseStatements env errors allStmts blkType =  q env errors allStmts []
        where
            ...
            q env errors (s:xs) annStmts = do
                (env1, errors1, annStmt) <- parseStatement s env errors blkType
                q env1 errors1 xs (annStmts++[annStmt])


parseStatement :: Stmt stmtenv infType -> Env -> Errors -> BlockType -> SSAState (Env, Errors, Stmt Env Type)
parseStatement stmt env errs blkType = case stmt of
            ...
            -- Blocco
            (StmtComp beblock) -> do
                (env2, err2, block) <- parseBEBlock env errs beblock AnonymousBlk
...
parseIter (StmtIter (StmtWhileDo expr stmt)) env errs = do
    ...
    (newEnv, newErrs, parsedStmt) <- parseStatement stmt env1 errs1 AnonymousBlk
...
parseIter (StmtIter (StmtRepeat stmt expr)) env errs = do
    (env1, errs1, parsedStmt) <- parseStatement stmt env errs AnonymousBlk
...
parseSelection (StmtSelect (StmtIfElse expr stmt1 stmt2)) env errs = do
    (env1, errs1, parsedExpr, posEnds) <- parseExpression env errs expr
    (newEnv1, newErrs1, parsedStmt1) <- parseStatement stmt1 env1 errs1 AnonymousBlk
    let wrappedStmt1 = wrapInBeginEnd parsedStmt1 newEnv1
    (newEnv2, newErrs2, parsedStmt2) <- parseStatement stmt2 newEnv1 newErrs1 AnonymousBlk
