...
-- last boolean argument is true if calling from parseDclVrBlock, false otherwise
    -- it is needed to add to uninitVars only variables declared in a var block
    -- excluding for example those of a function declaration which are initialized in the function body 
    -- TODO: implement check for variable initialization in function body
parseIds :: [IdElem] -> Modality -> Type -> Env -> Errors -> Bool -> SSAState (Env, Errors, [IdElem])
parseIds [] mod typ env errs _ = return (env, errs, [])
parseIds ( idElem@(IdElement (TokIdent (pos, id))):ids) mod typ env errs fromVrBlock = do
    idAddr <- newIdAddr id env
    tmpEnv <- Env.insert id (VarType mod pos typ idAddr) env
    (newEnv, newErrs, newIds) <- parseIds ids mod typ tmpEnv errs fromVrBlock

    -- add variable to unInitVars in state. It will be removed when the variable is initialized 
    -- TODO: variables of any type can be added to uninitVars. 
    --       Understand if this is correct or if we want to only add certain types (at least for now)
    Control.Monad.when fromVrBlock $ do
            state <- get;
            put $ state {uninitVars = Map.insert id (VarType mod pos typ idAddr) (uninitVars state) };

    -- state <- get;
    -- traceM ("UNINIT VARS IN STATE : " ++ show (uninitVars state))
